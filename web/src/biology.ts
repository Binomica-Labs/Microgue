...
export type GeneId =
  ...
  | "chiA" | "celA" | "dspB" | "epsA" | "comA" | "aprE" | "tssB" | "merA" | "czcA" | "copA" | "acrB" | "motA" | "fliC" | "cheY"
  | "cydA" | "cbbM" | "nifD" | "anfG" | "phsA" | "ttrA" | "frdA" | "cooS" | "pceA" | "arsC" | "arrA" | "mnhA" | "atpB" | "groL" | "dnaK" | "recA" | "uvrA" | "cheA" | "flhD" | "pilA"
...
/**
 * Pathways are the COLOURS. Same-pathway neighbours in one operon co-regulate
 * and earn a synergy bonus, so a pathway is a build axis: an operon of one
 * colour is the easy, strong thing to make.
 *
 * `defense` used to be a catch-all of sixteen genes -- repair, motility,
 * secretion and resistance all one colour, so a "defense operon" meant
 * nothing. Split into what those genes actually do, each its own colour and
 * its own build:
 *
 *   stress     oxidative/thermal/UV repair -- the tank
 *   motility   chemotaxis, flagella -- the skirmisher (dash, escape)
 *   secretion  enzymes, matrix, competence -- the controller (tiles, biofilm)
 *   resist     toxin/metal detox -- the deep-diver
 */
export type Pathway =
  | "photo" | "carbon" | "nitrogen" | "sulfur" | "iron"
  | "methane" | "energy" | "core"
  | "stress" | "motility" | "secretion" | "resist";
...
export const GENES: Readonly<Record<GeneId, Gene>> = {
  ...
  comA: { id:"comA", name:"comA", kb:1.0, product:"competence DNA uptake channel", tier:3, desc:"Take up ambient DNA from the water. Sometimes it is useful.", discovery:"Natural competence lets bacteria like Bacillus and Haemophilus import naked DNA straight from their surroundings -- how Griffith's 1928 transforming principle worked, decades before anyone knew it was DNA.", pathway:"secretion" },
  tssB: { id:"tssB", name:"tssB", kb:1.4, product:"Type VI secretion sheath", tier:3, desc:"A contractile sheath that fires a spear into a neighbouring cell.", discovery:"The T6SS is an inverted bacteriophage tail: the same machine, anchored inside the cell and fired outward. Vibrio cholerae uses it to kill competitors on contact.", pathway:"secretion" },
  aprE: { id:"aprE", name:"aprE", kb:1.1, product:"subtilisin protease", tier:3, desc:"Secreted protease. Lays a sheet that digests what crosses it.", discovery:"Subtilisin from Bacillus subtilis is the workhorse industrial protease -- it is in most laundry detergent. The cell secretes it to strip protein from its surroundings.", pathway:"secretion" },
  epsA: { id:"epsA", name:"epsA", kb:1.3, product:"exopolysaccharide synthase", tier:2, desc:"Secretes the polymer that builds a biofilm. Hold ground with it.", discovery:"The eps gene clusters build the extracellular matrix that is most of a biofilm by volume -- the cells are a minority of what you see.", pathway:"secretion" },
  dspB: { id:"dspB", name:"dspB", kb:1.1, product:"dispersin B, a glycoside hydrolase", tier:1, desc:"Dissolves the poly-GlcNAc that holds a biofilm together.", discovery:"Dispersin B was described by Kaplan and colleagues in 2003, from a bacterium that lives in dental plaque. It dissolves the very biofilm its host builds.", pathway:"secretion" },
  ...
  merA: { id:"merA", name:"merA", kb:1.7, product:"mercuric reductase", tier:5, desc:"Reduces Hg(II) to volatile Hg(0), which simply leaves. Mercury resistance.", discovery:"The mer operon is the textbook plasmid-borne resistance: it spread through hospital bacteria on transposons in the 1970s.", pathway:"resist" },
  czcA: { id:"czcA", name:"czcA", kb:3.2, product:"Co/Zn/Cd efflux pump", tier:5, desc:"Pumps cobalt, zinc and cadmium out against a gradient. Heavy-metal resistance.", discovery:"The czc system from Ralstonia metallidurans, isolated from a zinc-smelter sediment, was the first characterised heavy-metal efflux pump.", pathway:"resist" },
  copA: { id:"copA", name:"copA", kb:2.5, product:"copper-exporting ATPase", tier:4, desc:"Exports Cu(I) from the cytoplasm. Copper is essential and lethal; this keeps it out.", discovery:"CopA is a P-type ATPase; its human homologues are the Wilson and Menkes disease genes.", pathway:"resist" },
  acrB: { id:"acrB", name:"acrB", kb:3.1, product:"multidrug efflux pump", tier:3, desc:"Pumps a broad range of toxic compounds out of the cell. The generalist exporter.", discovery:"AcrAB-TolC is the best-studied multidrug efflux system and a main reason E. coli shrugs off so many antibiotics.", pathway:"resist" },
  arsC: { id:"arsC", name:"arsC", kb:0.7, product:"arsenate reductase", tier:4, desc:"Reduces arsenate to arsenite so it can be pumped out. Arsenic resistance, not metabolism.", discovery:"The ars operon was among the first bacterial resistance systems sequenced, in the early 1980s.", pathway:"resist" },
  ...
  groL: { id:"groL", name:"groL", kb:1.7, product:"chaperonin GroEL", tier:2, desc:"A folding chamber. Nothing large folds reliably at depth without help.", discovery:"GroEL's barrel structure was solved in 1994 and settled a long argument about whether folding needed machinery at all.", pathway:"stress" },
  dnaK: { id:"dnaK", name:"dnaK", kb:1.9, product:"Hsp70 chaperone", tier:3, desc:"Holds unfolded protein until it can be dealt with. The stress response's first responder.", discovery:"The heat shock response was described in the 1960s in fruit flies and turned out to be near-universal, dnaK included.", pathway:"stress" },
  recA: { id:"recA", name:"recA", kb:1.1, product:"recombinase A", tier:3, desc:"Repairs broken DNA by homologous recombination, and switches on the SOS response when things get bad.", discovery:"RecA was purified in 1979 and is the ancestor of every recombinase since, including the eukaryotic RAD51.", pathway:"stress" },
  uvrA: { id:"uvrA", name:"uvrA", kb:2.8, product:"excision repair subunit A", tier:4, desc:"Finds and excises damaged bases. Ultraviolet is a surface problem, and the surface is where light is.", discovery:"Nucleotide excision repair was worked out in the 1960s; Sancar shared the 2015 Nobel for mapping it in detail.", pathway:"stress" },
  motA: { id:"motA", name:"motA", kb:0.9, product:"flagellar motor stator", tier:2, desc:"The stator of the flagellar motor: converts proton flow into torque.", discovery:"MotA/MotB form the ion channel that drives the flagellum; the motor turns at hundreds of revolutions a second.", pathway:"motility" },
  fliC: { id:"fliC", name:"fliC", kb:1.5, product:"flagellin", tier:2, desc:"The filament protein itself. Without it the motor spins nothing.", discovery:"Flagellin is the most abundant protein in a motile E. coli, and its H-antigen is what serotyping reads.", pathway:"motility" },
  cheY: { id:"cheY", name:"cheY", kb:0.4, product:"chemotaxis response regulator", tier:2, desc:"Binds the motor and reverses it: the run-and-tumble switch.", discovery:"CheY-P is the single output of bacterial chemotaxis, the simplest signalling network fully worked out.", pathway:"motility" },
  cheA: { id:"cheA", name:"cheA", kb:2.0, product:"chemotaxis histidine kinase", tier:4, desc:"The kinase at the centre of chemotaxis. Tells the flagellum which way not to go.", discovery:"Bacterial chemotaxis became the model two-component system after Adler's experiments in the 1960s showed bacteria genuinely choose.", pathway:"motility" },
  flhD: { id:"flhD", name:"flhD", kb:0.4, product:"flagellar master regulator", tier:2, desc:"Decides whether to build flagella at all. Motility is expensive and not always worth it.", discovery:"The flagellar regulon is one of the largest coordinated gene programmes known, and flhDC sits at the top of it.", pathway:"motility" },
  ...
  otsA: { id:"otsA", name:"otsA", kb:1.4, product:"trehalose-6-phosphate synthase", tier:3, desc:"Stack trehalose against osmotic shock. Survive drying out.", discovery:"Trehalose lets tardigrades and resurrection plants survive desiccation by replacing the water around their proteins -- vitrification rather than repair.", pathway:"stress" },
  cspA: { id:"cspA", name:"cspA", kb:0.4, product:"cold shock protein A", tier:2, desc:"Keep RNA from folding shut in the cold. Cheap and always useful.", discovery:"CspA can reach a tenth of total cell protein within minutes of a temperature drop, one of the fastest known responses to a physical stress.", pathway:"stress" },
...
export const MICROBES: readonly Microbe[] = [
  ...
  { id:"bdellovibrio",   name:"Bdellovibrio",   depth:1, weight:0.25, hp:7,  atk:3,  glyph:"b", genes:["flhD","recA","aprE","sodA","fliC","cheY"], note:"Predatory bacterium. Hunts other cells; backs off when hurt." , pigment:"#e0a37a" , facing:"rotate" , behaviour:"hunt", size:"small" , weapon:"melee" },
  ...
  { id:"nitrobacter",     name:"Nitrobacter",     depth:2, hp:9,  atk:3,  glyph:"N", genes:["nxrA","cyoA","cydA","frdA"],        note:"Nitrite oxidiser. Completes nitrification." , pigment:"#bfae8e" , facing:"rotate" , behaviour:"drift", size:"small" , weapon:"melee" },
  { id:"pseudomonas",     name:"Pseudomonas",     depth:2, hp:12, atk:4,  glyph:"p", genes:["narG","nirS","norB","nosZ","dspB","napA","nrfA","ccoN","frdA","flhD","cheA","arsC","epsA","aprE","motA","fliC","cheY","tssB"], note:"Facultative denitrifier. Carries the whole chain." , pigment:"#cfe04a" , facing:"rotate" , behaviour:"hunt", size:"medium" , weapon:"spear" },
  { id:"beggiatoa",       name:"Beggiatoa",       depth:3, hp:16, atk:5,  glyph:"B", genes:["soxB","sqr","chiA","cymA","cydA","phsA"],  note:"Gliding sulfur mat. Stores S0 granules internally." , pigment:"#f2f2e6" , facing:"rotate" , behaviour:"glide", size:"filament" , weapon:"melee" },
  { id:"thiothrix",       name:"Thiothrix",       depth:3, hp:14, atk:5,  glyph:"t", genes:["soxB","phsA","groL","epsA"],        note:"Filamentous, rosette-forming sulfur oxidiser." , pigment:"#e6e6da" , facing:"none" , behaviour:"sessile", size:"filament" , weapon:"melee" },
  { id:"vampirovibrio",   name:"Vampirovibrio",   depth:3, hp:14, atk:8,  glyph:"v", genes:["mtrC","katG","recA","sodA","aprE","tssB"], note:"Predatory bacterium. Lies still, then strikes." , pigment:"#c48fb0" , facing:"rotate" , behaviour:"ambush", size:"small" , weapon:"melee" },
  { id:"thiobacillus",    name:"Thiobacillus",    depth:3, hp:11, atk:6,  glyph:"T", genes:["sqr","soxB","ttrA","arsC","merA","acrB"],  note:"Chemolithoautotroph. Generates sulfuric acid." , pigment:"#d8cfa0" , facing:"rotate" , behaviour:"drift", size:"small" , weapon:"cloud" },
  { id:"geobacter",       name:"Geobacter",       depth:4, hp:18, atk:7,  glyph:"G", genes:["omcS","mtrC","dspB","mtrB","cymA","pilA","arrA","epsA","copA","acrB"], note:"Grows conductive pili. Reduces solid Fe(III) oxides." , pigment:"#d0603c" , facing:"rotate" , behaviour:"wire", size:"medium" , weapon:"bolt" },
  { id:"shewanella",      name:"Shewanella",      depth:4, hp:16, atk:6,  glyph:"S", genes:["mtrC","mtrB","cymA","mtoA","arrA","ttrA","frdA","comA","cheY","motA","acrB","tssB"],        note:"Mtr pathway respires minerals. Wildly versatile." , pigment:"#dd9078" , facing:"rotate" , behaviour:"flank", size:"medium" , weapon:"melee" },
  { id:"rhodospirillum",  name:"Rhodospirillum",  depth:4, hp:15, atk:5,  glyph:"r", genes:["pufM","nifH","crtI","cbbM","cooS","nifD","motA","fliC"], note:"Purple non-sulfur. Photoheterotroph, fixes N2." , pigment:"#b0527a" , facing:"rotate" , behaviour:"chase", size:"medium" , weapon:"melee" },
  ...
  { id:"desulfovibrio",   name:"Desulfovibrio",   depth:7, hp:28, atk:11, glyph:"D", genes:["dsrA","hydA","dsrB","qmoA","hynL","pceA","mnhA","otsA","czcA","copA"], note:"Sulfate reducer. Exhaled H2S blackens the sediment." , pigment:"#a6acb6" , facing:"rotate" , behaviour:"leech", size:"medium" , weapon:"cloud" },
  ...
  { id:"methanosarcina",  name:"Methanosarcina",  depth:8, hp:36, atk:14, glyph:"M", genes:["mcrA","hdrB","cdhA","ackA","fwdB","cooS","mnhA","recA","frhA","mtrH","acsB","czcA","merA"], note:"The most metabolically flexible methanogen known." , pigment:"#dcc179" , facing:"none" , behaviour:"sessile", size:"large" , weapon:"packet" },
  ...
  { id:"methylomonas",    name:"Methylomonas",    depth:7, hp:26, atk:10,  glyph:"m", genes:["pmoA","mmoX","katG","sodA","atpB","groL"], note:"Methanotroph. Oxidises the methane rising from below." , pigment:"#9fd8a8" , facing:"rotate" , behaviour:"hunt", size:"medium" , weapon:"melee" },
