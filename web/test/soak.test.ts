...
describe("state that should persist, does", () => {
  ...
  const TRANSIENT: Record<string, string> = {
    ...
    webView: "the bench map's pan and zoom, per session",
    webPick: "which gene the map has selected",
    webHitAt: "hit-test closure, rebuilt each frame",
    ...
    logH: "layout, per frame", logMaxH: "layout, per frame", sizedFor: "the viewport, re-read every frame", turnSeed: "derived from the turn",
